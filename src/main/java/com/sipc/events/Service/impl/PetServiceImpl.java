...
public class PetServiceImpl implements PetService {
    ...
    public void adoptPet(MessageEventParam messageEventParam) {
        ...
        String petName = message.substring(message.indexOf("养") + 1).trim();
    ...
    public void changeName(MessageEventParam messageEventParam) {
        String message = messageEventParam.getMessage();
        String petName = message.substring(message.indexOf("名") + 1).trim();
        QueryWrapper<PetPO> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("user_id", messageEventParam.getUser_id());
        queryWrapper.eq("group_id", messageEventParam.getGroup_id());
    ...
    public void hug(MessageEventParam messageEventParam) {
        ...
        String msg = "贴贴~" + "\n" +
                petPO.getPetName() + ": " + String.format("%.2f", petPO.getPetSize()) + "\n" +
                petCouplePo.getPetName() + ": " + String.format("%.2f", petCouplePo.getPetSize());
    ...
    public void fight(MessageEventParam messageEventParam) {
        ...
        if (isArrest) {
            ...
            String msg = "你们被逮捕了！！！" + "\n" +
                    petPO.getPetName() + ":" + String.format("%.2f", petPO.getPetSize()) + "\n" +
                    enemyPetPO.getPetName() + ":" + String.format("%.2f", enemyPetPO.getPetSize());
            petDao.updateById(petPO);
        ...
        if (petAtk > +enemyAtk) {
            ...
            petDao.updateById(petPO);
            petDao.updateById(enemyPetPO);
            String msg = petPO.getPetName() + "胜利啦！！！" + "\n" +
                    petPO.getPetName() + "的size: " + String.format("%.2f", petPO.getPetSize()) + "\n" +
                    petPO.getPetName() + "的atk: " + String.format("%.2f", petAtk) + "\n" +
                    petPO.getPetName() + "变大了: " + String.format("%.2f", petVariation) + "\n" +
                    enemyPetPO.getPetName() + "的size: " + String.format("%.2f", enemyPetPO.getPetSize()) + "\n" +
                    enemyPetPO.getPetName() + "的atk: " + String.format("%.2f", enemyAtk) + "\n" +
                    enemyPetPO.getPetName() + "变小了: " + String.format("%.2f", enemyVariation);
            sendGroupMsg(messageEventParam.getGroup_id(), String.valueOf(msg), false);
        } else {
            ...
            petDao.updateById(petPO);
            petDao.updateById(enemyPetPO);
            String msg = enemyPetPO.getPetName() + "胜利啦！！！" + "\n" +
                    petPO.getPetName() + "的size: " + String.format("%.2f", petPO.getPetSize()) + "\n" +
                    petPO.getPetName() + "的atk: " + String.format("%.2f", petAtk) + "\n" +
                    petPO.getPetName() + "变小了: " + String.format("%.2f", petVariation) + "\n" +
                    enemyPetPO.getPetName() + "的size: " + String.format("%.2f", enemyPetPO.getPetSize()) + "\n" +
                    enemyPetPO.getPetName() + "的atk: " + String.format("%.2f", enemyAtk) + "\n" +
                    enemyPetPO.getPetName() + "变大了: " + String.format("%.2f", enemyVariation);
            sendGroupMsg(messageEventParam.getGroup_id(), String.valueOf(msg), false);
        }
    }
